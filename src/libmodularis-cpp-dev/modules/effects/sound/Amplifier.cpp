...
extern "C" void MDLRS_Amplifier_new_body(MDLRS::Amplifier *self);

namespace MDLRS
{
	Amplifier::Amplifier(Modularis &project, float _volume): Module(project), input(this), volume(this, _volume), output(this)
	{
		MDLRS_Amplifier_new_body(this);
	}
	void Amplifier::on_update()
	{
		output.frame=volume.value*input.frame;
	}
}