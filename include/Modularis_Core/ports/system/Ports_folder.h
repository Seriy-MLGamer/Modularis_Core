...
struct MDLRS_Ports_folder
{
	...
	MDLRS_Port_base **ports;
	uint32_t ports_size;
	uint32_t ports_count;
};
void MDLRS_Ports_folder_new(MDLRS_Ports_folder *self);
int
	MDLRS_Ports_folder_connect(MDLRS_Ports_folder *self, MDLRS_Port_base *port),
	...
	MDLRS_Ports_folder_disconnect_from_port(MDLRS_Ports_folder *self, MDLRS_Port_base *port),
...
inline void MDLRS_Ports_folder_add(MDLRS_Ports_folder *self, MDLRS_Port_base *port)
{
	if (self->ports)
	{
		if (self->ports_count==self->ports_size) self->ports=realloc(self->ports, sizeof(MDLRS_Port_base *)*(self->ports_size+=8));
		self->ports[self->ports_count++]=port;
	}
	else
	{
		self->ports=malloc(sizeof(MDLRS_Port_base *)*8);
		*self->ports=port;
		self->ports_size=8;
		self->ports_count=1;
	}
