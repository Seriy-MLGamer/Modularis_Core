...
{
	MDLRS_Port_new((MDLRS_Port *)self, "Real_controller", module);
	self->f=&f;
...
void MDLRS_Real_controller_on_update(MDLRS_Real_controller *self)
{
	...
	{
		self->value=0;
		for (uint32_t a=0; a!=self->connections_count; a++) self->value+=((MDLRS_Real_controller *)self->connections[a].port)->value;
