...
extern "C" void MDLRS_Modulator_new_body(MDLRS::Modulator *self);

namespace MDLRS
{
	Modulator::Modulator(Modularis &project): Module(project), carrier(this), modulator(this), output(this)
	{
		MDLRS_Modulator_new_body(this);
	}
	void Modulator::on_update()
	{
		if (modulator.connections_count) output.frame=modulator.frame*carrier.frame;
		else output.frame=carrier.frame;
	}
}