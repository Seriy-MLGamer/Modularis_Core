...
#include <Modularis_Core_C++/user/modules/players/Sequencer/Pattern_none.hpp>

extern "C" void MDLRS_Pattern_none_new(MDLRS::Pattern_none *self, float length);

namespace MDLRS
{
	Pattern_none::Pattern_none(float length)
	{
		MDLRS_Pattern_none_new(this, length);
	}
	uint32_t Pattern_none::get_tracks_count()
	{
		return 0;
	}
	void Pattern_none::null_position(Pattern_data &data){}
	void Pattern_none::change_position(Pattern_data &data, float time){}
	void Pattern_none::start(Note &output, Pattern_data &data, uint32_t scancode, float time){}
	void Pattern_none::play(Note &output, Pattern_data &data, uint32_t scancode, float time){}
}