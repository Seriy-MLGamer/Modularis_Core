...
{
	(int (*)(void *, MDLRS_Port_base *))MDLRS_Ports_folder_connect,
	...
	(int (*)(void *, MDLRS_Port_base *))MDLRS_Ports_folder_disconnect_from_port,
...
int MDLRS_Ports_folder_connect(MDLRS_Ports_folder *self, MDLRS_Port_base *port)
{
	return ((MDLRS_Port_base_f)port->f)->connect_folder(port, self);
}
...
int MDLRS_Ports_folder_connect_folder(MDLRS_Ports_folder *self, MDLRS_Ports_folder *folder)
{
	...
	for (uint32_t a=0; a!=minimum_count; a++)
	{
		...
		((MDLRS_Port_base_f)port->f)->connect(port, self->ports[a]);
	}
	return 0;
}
int MDLRS_Ports_folder_disconnect(MDLRS_Ports_folder *self)
{
	...
	{
		MDLRS_Port_base *port=self->ports[a];
		((MDLRS_Port_base_f)port->f)->disconnect(port);
	}
	return 0;
}
int MDLRS_Ports_folder_disconnect_from_port(MDLRS_Ports_folder *self, MDLRS_Port_base *port)
{
	return ((MDLRS_Port_base_f)port->f)->disconnect_folder(port, self);
}
...
int MDLRS_Ports_folder_disconnect_folder(MDLRS_Ports_folder *self, MDLRS_Ports_folder *folder)
{
	...
	{
		MDLRS_Port_base *port=folder->ports[a];
		((MDLRS_Port_base_f)port->f)->disconnect_from_port(port, self->ports[a]);
	}
	return 0;
}
int MDLRS_Ports_folder_disconnect_input(MDLRS_Ports_folder *self)
{
	...
	{
		MDLRS_Port_base *port=self->ports[a];
		((MDLRS_Port_base_f)port->f)->disconnect_input(port);
	}
	return 0;
}
void MDLRS_Ports_folder_update(MDLRS_Ports_folder *self)
{
	...
	{
		MDLRS_Port_base *port=self->ports[a];
		((MDLRS_Port_base_f)port->f)->update(port);
	}
}
void MDLRS_Ports_folder_get_ready(MDLRS_Ports_folder *self)
{
	...
	{
		MDLRS_Port_base *port=self->ports[a];
		((MDLRS_Port_base_f)port->f)->get_ready(port);
	}
}